...
import { Agent, Plan, money } from '@/lib/data';
...
export default function Checkout({ agentId, initialPlan, onBack, onConfirm }: CheckoutProps) {
  ...
  if (!agent) return (
    <main style={{ maxWidth: 980, margin: '0 auto', padding: '80px 32px', textAlign: 'center', color: '#A89E91' }}>Loading…</main>
  );
  ...
  return (
    <main className="fade-up" style={{ maxWidth: 980, margin: '0 auto', padding: '28px 32px 90px' }}>
      ...
      <h1 style={{ fontSize: 30, fontWeight: 800, letterSpacing: '-0.025em', margin: '0 0 28px' }}>Confirm &amp; hire {agent.name}</h1>

      <div style={{ display: 'grid', gridTemplateColumns: '1fr 360px', gap: 32, alignItems: 'start' }}>
        <div>
          <div style={{ fontSize: 13, fontWeight: 600, color: '#867C71', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 13 }}>1 · Select plan</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 30 }}>
            {planDefs.map(p => {
              const sel = p.key === plan;
              return (
                <div key={p.key} onClick={() => setPlan(p.key)} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '16px 18px', borderRadius: 13, cursor: 'pointer', border: '1px solid ' + (sel ? '#DD6A33' : '#ECE5DA'), background: sel ? '#FEF8F4' : '#fff', boxShadow: sel ? '0 0 0 3px rgba(221,106,51,0.10)' : 'none' }}>
                  <div style={{ width: 20, height: 20, borderRadius: '50%', flex: 'none', border: '2px solid ' + (sel ? '#DD6A33' : '#D8CFC2'), background: sel ? 'radial-gradient(circle, #DD6A33 0 5px, #fff 6px)' : '#fff' }} />
                  <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: 13, fontWeight: 600, color: '#867C71', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 13 }}>2 · Payment</div>
          <div style={{ background: '#fff', border: '1px solid #ECE5DA', borderRadius: 14, padding: 20, display: 'flex', flexDirection: 'column', gap: 15 }}>
            <div>
              ...
              <input placeholder="4242 4242 4242 4242" style={{ width: '100%', border: '1px solid #E4DCD0', borderRadius: 10, padding: '12px 14px', fontSize: 15, fontFamily: "'JetBrains Mono', monospace" }} />
            </div>
            <div style={{ display: 'flex', gap: 12 }}>
              <div style={{ flex: 1 }}><label style={{ display: 'block', fontSize: 13, fontWeight: 600, marginBottom: 6 }}>Expiry</label><input placeholder="12 / 28" style={{ width: '100%', border: '1px solid #E4DCD0', borderRadius: 10, padding: '12px 14px', fontSize: 15, fontFamily: "'JetBrains Mono', monospace" }} /></div>
              <div style={{ flex: 1 }}><label style={{ display: 'block', fontSize: 13, fontWeight: 600, marginBottom: 6 }}>CVC</label><input placeholder="123" style={{ width: '100%', border: '1px solid #E4DCD0', borderRadius: 10, padding: '12px 14px', fontSize: 15, fontFamily: "'JetBrains Mono', monospace" }} /></div>
            </div>
          </div>
        </div>
        ...
        <aside style={{ position: 'sticky', top: 88, background: '#fff', border: '1px solid #ECE5DA', borderRadius: 18, padding: 22, boxShadow: '0 8px 30px rgba(43,37,32,0.06)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, paddingBottom: 16, borderBottom: '1px solid #F1EBE2', marginBottom: 16 }}>
            ...
            <div><div style={{ fontWeight: 700, fontSize: 15 }}>{agent.name}</div><div style={{ fontSize: 13, color: '#867C71' }}>{orderPlanLabel}</div></div>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10, fontSize: 14, color: '#5E554B' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}><span>{orderLine1Label}</span><span style={{ fontWeight: 600, color: '#2B2520' }}>{orderLine1Val}</span></div>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}><span>Platform fee</span><span style={{ fontWeight: 600, color: '#2B2520' }}>{fee}</span></div>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}><span>Tax</span><span style={{ fontWeight: 600, color: '#2B2520' }}>{tax}</span></div>
          </div>
          ...
          <button onClick={onConfirm} style={{ width: '100%', marginTop: 18, background: '#DD6A33', color: '#fff', border: 'none', borderRadius: 12, padding: 15, fontWeight: 700, fontSize: 15.5, cursor: 'pointer' }}>{confirmLabel}</button>
