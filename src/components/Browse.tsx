...
import { AGENTS, CATEGORIES, Agent, fmt, money } from '@/lib/data';
...
export default function Browse({ onOpen }: BrowseProps) {
  ...
  let list = AGENTS.filter(a => {
    ...
    if (search) {
      ...
      const hay = (a.name + ' ' + a.blurb + ' ' + a.category + ' ' + a.tags.join(' ')).toLowerCase();
      if (!hay.includes(q)) return false;
    }
    return true;
  });

  if (sort === 'popular') list = [...list].sort((a, b) => b.tasks - a.tasks);
  else if (sort === 'rating') list = [...list].sort((a, b) => b.rating - a.rating);
  else if (sort === 'priceLow') list = [...list].sort((a, b) => a.monthly - b.monthly);
  else if (sort === 'priceHigh') list = [...list].sort((a, b) => b.monthly - a.monthly);

  const resultLabel = list.length + (list.length === 1 ? ' agent' : ' agents') + (category !== 'All' ? ' in ' + category : ' available');
    ...
    <main className="fade-up">
      ...
      <section style={{ maxWidth: 1200, margin: '0 auto', padding: '64px 32px 40px' }}>
        <div style={{
          ...
          padding: '6px 12px', borderRadius: 999, background: '#FBEDE4', color: '#C0532A',
          ...
          1,240 agents online now
        </div>

        <h1 style={{
          fontSize: 54, lineHeight: 1.04, letterSpacing: '-0.035em', fontWeight: 800,
        ...
        <div style={{ display: 'flex', gap: 12, maxWidth: 640, marginBottom: 14 }}>
          <div style={{
            ...
            background: '#fff', border: '1px solid #E4DCD0', borderRadius: 12,
            padding: '0 16px', height: 54, boxShadow: '0 1px 2px rgba(43,37,32,0.03)',
          }}>
          ...
          <select
            ...
            style={{
              height: 54, border: '1px solid #E4DCD0', borderRadius: 12,
              background: '#fff', padding: '0 16px', fontSize: 15, fontWeight: 500, color: '#2B2520', cursor: 'pointer',
            }}
          >
            ...
            return (
              <button
                ...
                style={{
                  ...
                  border: '1px solid ' + (active ? '#DD6A33' : '#E4DCD0'),
                  background: active ? '#DD6A33' : '#fff',
                  color: active ? '#fff' : '#5E554B',
                }}
              >
                {c}
              </button>
      ...
      <section style={{ maxWidth: 1200, margin: '0 auto', padding: '8px 32px 80px' }}>
        <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 20 }}>
          <h2 style={{ fontSize: 15, fontWeight: 600, color: '#756B61', margin: 0, textTransform: 'uppercase', letterSpacing: '0.06em' }}>
            {resultLabel}
          </h2>
        </div>

        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 20 }}>
          {list.map(a => <AgentCard key={a.id} agent={a} onOpen={onOpen} />)}
        </div>
      </section>
    </main>
  );
...
function AgentCard({ agent: a, onOpen }: { agent: Agent; onOpen: (id: string) => void }) {
  ...
  return (
    <div
      ...
      style={{
        background: '#fff',
        border: '1px solid ' + (hovered ? '#E0D2C2' : '#ECE5DA'),
        borderRadius: 16, padding: 22, cursor: 'pointer',
        display: 'flex', flexDirection: 'column', gap: 14,
        transition: 'box-shadow 0.18s ease, transform 0.18s ease, border-color 0.18s ease',
        boxShadow: hovered ? '0 12px 28px rgba(43,37,32,0.10)' : 'none',
    ...
    >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
            ...
            {a.verified && (
              <span title="Verified seller" style={{
                ...
                width: 16, height: 16, borderRadius: '50%', background: '#3F9D6B',
      ...
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
        {a.tags.map(t => (
          <span key={t} style={{
            ...
            color: '#867C71', background: '#F4EEE5', padding: '3px 8px', borderRadius: 6,
          }}>{t}</span>
        ))}
      </div>
      ...
      <div style={{
        ...
        paddingTop: 13, borderTop: '1px solid #F1EBE2', fontSize: 13, color: '#756B61',
      }}>
        ...
        <div style={{ flex: 1 }} />
        <span style={{ fontWeight: 700, color: '#2B2520' }}>
          {money(a.monthly)}<span style={{ color: '#A89E91', fontWeight: 400 }}>/mo</span>
        </span>
      </div>
    </div>
  );
