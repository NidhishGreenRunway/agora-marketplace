...
import { Agent, Review, Plan, fmt, money } from '@/lib/data';
...
export default function ListingDetail({ agentId, onBack, onCheckout }: ListingDetailProps) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/agents/${agentId}`)
      .then(r => r.json())
      .then(data => { setAgent(data.agent); setReviews(data.reviews ?? []); })
  ...
  if (loading) return (
    <main style={{ maxWidth: 1200, margin: '0 auto', padding: '80px 32px', textAlign: 'center', color: '#A89E91' }}>
      Loading agent…
    </main>
  );
  ...
  if (!agent) return (
    <main style={{ maxWidth: 1200, margin: '0 auto', padding: '80px 32px', textAlign: 'center', color: '#A89E91' }}>
      Agent not found. <button onClick={onBack} style={{ color: '#DD6A33', background: 'none', border: 'none', cursor: 'pointer', fontWeight: 600 }}>Back to marketplace</button>
    </main>
  );
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 360px', gap: 40, alignItems: 'start' }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 18, marginBottom: 26 }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
                ...
                {agent.verified && <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, background: '#E6F1EA', color: '#2E7A50', fontSize: 12, fontWeight: 600, padding: '4px 10px', borderRadius: 999 }}>✓ Verified seller</span>}
              </div>
          ...
          <div style={{ marginBottom: 34 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 13 }}>
              ...
              <span style={{ fontSize: 11, fontFamily: "'JetBrains Mono', monospace", background: '#FBEDE4', color: '#C0532A', padding: '2px 8px', borderRadius: 6 }}>FREE DEMO</span>
            </div>
                ...
                {demoMsgs.map((m, i) => {
                  ...
                  return (
                    <div key={i} style={{ display: 'flex', justifyContent: mine ? 'flex-end' : 'flex-start' }}>
                      <div style={{ maxWidth: '82%', padding: '11px 14px', borderRadius: mine ? '14px 14px 4px 14px' : '14px 14px 14px 4px', fontSize: 14.5, lineHeight: 1.45, background: mine ? '#2B2520' : '#F4EEE5', color: mine ? '#fff' : '#3A332C' }}>{m.text}</div>
                    </div>
                  );
                })}
              </div>
              <div style={{ display: 'flex', gap: 10, padding: 13, borderTop: '1px solid #F1EBE2', background: '#FCFAF6' }}>
                <input value={demoInput} onChange={e => setDemoInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && sendDemo()} placeholder="Ask the agent to do something…" style={{ flex: 1, border: '1px solid #E4DCD0', borderRadius: 10, padding: '11px 14px', fontSize: 14.5, background: '#fff' }} />
                <button onClick={sendDemo} style={{ background: '#2B2520', color: '#fff', border: 'none', borderRadius: 10, padding: '0 20px', fontWeight: 600, fontSize: 14, cursor: 'pointer' }}>Send</button>
              </div>
            </div>
          </div>
          ...
          {reviews.length > 0 && (
            <div>
              <h3 style={{ fontSize: 18, fontWeight: 700, margin: '0 0 16px' }}>Reviews <span style={{ color: '#A89E91', fontWeight: 500 }}>({agent.reviews.toLocaleString()})</span></h3>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                {reviews.map((r, i) => (
                  <div key={i} style={{ background: '#fff', border: '1px solid #ECE5DA', borderRadius: 13, padding: 18 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 9 }}>
                      <div style={{ width: 30, height: 30, borderRadius: '50%', background: '#F0E6D8', color: '#8A7B66', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 700, fontFamily: "'JetBrains Mono', monospace" }}>{r.initials}</div>
        ...
        <aside style={{ position: 'sticky', top: 88 }}>
          <div style={{ background: '#fff', border: '1px solid #ECE5DA', borderRadius: 18, padding: 22, boxShadow: '0 8px 30px rgba(43,37,32,0.06)' }}>
            <div style={{ fontSize: 13, fontWeight: 600, color: '#867C71', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 16 }}>Choose how to hire</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {planDefs.map(p => {
                const sel = p.key === selectedPlan;
                return (
                  <div key={p.key} style={{ border: '1px solid ' + (sel ? '#DD6A33' : '#ECE5DA'), background: sel ? '#FEF8F4' : '#fff', borderRadius: 13, padding: 15, boxShadow: sel ? '0 0 0 3px rgba(221,106,51,0.10)' : 'none' }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
                      <span style={{ fontWeight: 700, fontSize: 15 }}>{p.label}</span>
                      {p.badge && <span style={{ fontSize: 10.5, fontFamily: "'JetBrains Mono', monospace", background: '#DD6A33', color: '#fff', padding: '2px 7px', borderRadius: 5 }}>{p.badge}</span>}
                    </div>
                    ...
                    <button onClick={() => { setSelectedPlan(p.key); onCheckout(p.key); }} style={{ width: '100%', marginTop: 12, borderRadius: 10, padding: 11, fontWeight: 700, fontSize: 14, cursor: 'pointer', border: 'none', background: sel ? '#DD6A33' : '#2B2520', color: '#fff' }}>{p.cta}</button>
                  </div>
                );
              })}
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 16, paddingTop: 16, borderTop: '1px solid #F1EBE2', fontSize: 12.5, color: '#867C71', lineHeight: 1.45 }}>
              <span style={{ color: '#3F9D6B', fontSize: 15 }}>✓</span> Money-back guarantee on the first 100 tasks. Cancel anytime.
            </div>
          </div>
        </aside>
      </div>
