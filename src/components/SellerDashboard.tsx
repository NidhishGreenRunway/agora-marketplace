...
export default function SellerDashboard({ onPublish }: SellerDashboardProps) {
  ...
  useEffect(() => {
    Promise.all(MY_IDS.map(id =>
      fetch(`/api/agents/${id}`).then(r => r.ok ? r.json() : null)
    )).then(results => {
      setMyAgents(results.filter(Boolean).map((d: { agent: Agent }) => d.agent));
    });
  }, []);
    ...
    <main className="fade-up" style={{ maxWidth: 1200, margin: '0 auto', padding: '40px 32px 90px' }}>
      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: 30 }}>
        <div>
          ...
          <p style={{ color: '#756B61', fontSize: 15, margin: 0 }}>Welcome back — here's how your agents are doing.</p>
        </div>
        <button onClick={onPublish} style={{ background: '#DD6A33', color: '#fff', border: 'none', borderRadius: 11, padding: '12px 20px', fontWeight: 700, fontSize: 14.5, cursor: 'pointer' }}>
          + Publish new agent
        </button>
      </div>
      ...
      <div style={{ background: '#fff', border: '1px solid #ECE5DA', borderRadius: 16, overflow: 'hidden' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '2.2fr 1fr 1fr 1fr 0.8fr', gap: 16, padding: '14px 22px', background: '#FCFAF6', borderBottom: '1px solid #F1EBE2', fontSize: 12, fontWeight: 600, color: '#867C71', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
          <span>Agent</span><span>Active renters</span><span>Tasks run</span><span>Monthly revenue</span><span>Status</span>
        </div>
        {myAgents.length === 0 && (
          <div style={{ padding: '32px 22px', color: '#A89E91', fontSize: 14 }}>Loading agents…</div>
        )}
        {myAgents.map(a => {
          const meta = SELLER_META[a.id] ?? { renters: 0, mrr: 0, tasks: '0' };
          return (
            <div key={a.id} style={{ display: 'grid', gridTemplateColumns: '2.2fr 1fr 1fr 1fr 0.8fr', gap: 16, padding: '18px 22px', borderBottom: '1px solid #F4EEE5', alignItems: 'center' }}>
              ...
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 13, color: '#2E7A50', fontWeight: 600 }}>
                <span style={{ width: 7, height: 7, borderRadius: '50%', background: '#3F9D6B', display: 'inline-block' }} />Live
              </span>
            </div>
          );
        })}
